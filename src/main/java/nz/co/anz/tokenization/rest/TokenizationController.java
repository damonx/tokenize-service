...
public class TokenizationController {
  ...
  public List<String> tokenize(
      @RequestBody @NotEmpty @Size(max = 50, message = "Maximum 50 account numbers per request")
          final List<
                  @Pattern(
                      regexp = "^(\\d{4}[-\\s]?){3}\\d{4}$",
  ...
  public List<String> detokenize(
      @RequestBody @NotEmpty @Size(max = 50, message = "Maximum 50 tokens per request")
          final List<@Pattern(regexp = "^[A-Za-z0-9]{32}$", message = "Wrong token format.") String>
              tokens) {
