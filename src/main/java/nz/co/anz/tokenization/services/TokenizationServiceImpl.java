...
public class TokenizationServiceImpl implements TokenizationService {
  ...
  public List<String> tokenize(final List<String> accountNumbers) {
    return accountNumbers.stream().map(this::tokenizeAccountNumber).toList();
  }

  /**
   * Tokenizes the provided account number by either retrieving an existing token from the database
   * or generating a new one.
   *
   * <p>If the account number has been tokenized previously, the existing token is returned.
   * Otherwise, a new 32-character alphanumeric token is generated, persisted to the repository
   * associated with the account number, and then returned.
   *
  ...
  public List<String> detokenize(final List<String> tokens) {
    return tokens.stream().map(tokenFinder::resolve).toList();
  }
}
