 ...
 *
 * <ul>
 *   <li>Clear and standardised error responses
 *   <li>Appropriate HTTP status codes
 *   <li>Useful diagnostic information for clients
 *   <li>Centralised logging of exceptional scenarios
 * </ul>
 *
 * <p>This class is applied globally via {@link RestControllerAdvice}.
 */
...
public class GlobalExceptionHandler {
   ...
   *
   * <p>Returns a {@code 404 Not Found} response with a descriptive error message.
   *
  ...
  /**
   * Handles {@link MethodArgumentNotValidException} triggered by request body validation failures
   * (e.g. {@code @Valid} annotated request payloads).
   *
   * <p>Aggregates all validation error messages and returns them as part of a {@code 400 Bad
   * Request} response.
   *
  ...
  /**
   * Handles {@link ConstraintViolationException} thrown by method-level validation constraints.
   *
   * <p>Each constraint violation message is collected and returned in a {@code 400 Bad Request}
   * response.
   *
   ...
   *
   * <p>This typically occurs when a request is sent without a body or contains malformed JSON.
   *
   ...
   *
   * <p>Returns a generic {@code 500 Internal Server Error} response and logs the exception for
   * diagnostic purposes.
   *
  ...
  /**
   * Builds a standard {@code 400 Bad Request} {@link ProblemDetail} for validation failures.
   *
