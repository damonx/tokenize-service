...
class PositiveDurationValidatorUnitTest {
  ...
  void validCacheDurationConfiguration() {
    // GIVEN
    // WHEN
    final boolean isValid = positiveDurationValidator.isValid(Duration.parse("PT5M"), context);
    // THEN
  ...
  void invalidCacheDurationConfigurations(final String duration) {
    // GIVEN
    // WHEN
    final boolean isValid = positiveDurationValidator.isValid(Duration.parse(duration), context);
    // THEN
  ...
  void nullCacheDurationConfiguration() {
    // GIVEN
    // WHEN
    final boolean isValid = positiveDurationValidator.isValid(null, context);
    // THEN
    assertThat(isValid).isFalse();
  }
}
