...
public class TokenizationIntegrationTest {
  ...
  void tokenizeFailsWhenTooManyAccountNumbers() {
    ...
    final List<String> accountNumbers =
        IntStream.range(0, 51).mapToObj(i -> "1234 5678 9012 3456").toList();
  ...
  void tokenizeCreatesTokenSuccessfully() {
    ...
    // WHEN
    final EntityExchangeResult<List<String>> tokensInResponse =
        webClient
            .post()
            .uri("/tokenize")
            .bodyValue(request)
  ...
  void tokenizeReturnsExistingToken() {
    ...
    final TokenEntity existingDbRecord = tokenRepository.findById(existingToken).orElseThrow();
    ...
    assertThat(tokenRepository.count()).as("Database size should still be 1").isEqualTo(1);
  }
}
