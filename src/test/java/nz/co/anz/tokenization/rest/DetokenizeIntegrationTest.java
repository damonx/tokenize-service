...
class DetokenizeIntegrationTest {
  ...
  @AfterEach
  void clearCache() {
    Optional.ofNullable(cacheManager.getCache("tokenToAccount")).ifPresent(Cache::clear);
  }
  ...
  void detokenizeFailsWhenTooManyTokens() {
    // GIVEN
    final List<String> tokens = IntStream.range(0, 51).mapToObj(i -> "A".repeat(32)).toList();
  ...
  @Test
  @DisplayName(
      "Detokenize successfully resolves a single token cache miss and result has been cached.")
  ...
  void detokenizeResolvesSingleTokenSuccessfully() {
    ...
    // WHEN
    final EntityExchangeResult<List<String>> result =
        webClient
            .post()
            .uri("/detokenize")
            .bodyValue(request)
  ...
  void detokenizeFailsWhenTokenNotFound() {
    ...
    assertThat(result.getResponseBody())
        .contains("Token not found: ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ");
  }
  ...
  void detokenizeResolvesMultipleTokens() {
    ...
    final List<String> request =
        List.of("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB");
